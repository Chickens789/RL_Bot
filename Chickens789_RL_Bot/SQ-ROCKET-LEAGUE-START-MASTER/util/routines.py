...
class jumper():
    def run(self, agent):
        print("Hippity Hoppity", int(agent.time), int(agent.time % 5))
        if agent.time % 5 == 0:
...
class atba(Routine):
    # An example routine that just drives towards the ball at max speed, uses boost
    def run(self, agent):
...
class kickoff(Routine):
    #Chickens - I want it to wait 1-2 seconds, then boost up, colliding with the ball midair and hit towards opponents goal location
...
class Ekickoff(Routine):
    def run(self, agent):
        if ((agent.me.location.x == 0) and (agent.me.location.y == -4608)) or ((agent.me.location.x == 0) and (agent.me.location.y == 4608)): #First is Blue Middle. Second is Red Middle.
            import time
            time.sleep(4) #Works
            print("Waited")
            self.set_intent(atba())
            print("Boost Toward Ball")

            print("Jump")
            print("Hit Towards Opponent Goal")
        elif ((agent.me.location.x == 256) and (agent.me.location.y == 3840)) or ((agent.me.location.x == -256) and (agent.me.location.y == -3840)): #First is Blue to Left of Ball. Second is Red to Left of Ball.
            print("Will do later, but know it spawns here.")
            print(agent.me.location)
        elif ((agent.me.location.x == 2048) and (agent.me.location.y == 2560)) or ((agent.me.location.x == -2048) and (agent.me.location.y == -2560)): # is Blue to the Left.  is Red to the Left.
            print("Will do later, but know it spawns here.")
            print(agent.me.location)
        elif ((agent.me.location.x == 2048) and (agent.me.location.y == -2560)) or ((agent.me.location.x == 2048) and (agent.me.location.y == -2560)): # is Blue to the Left.  is Red to the Left.
            print("Will do later, but know it spawns here.")
